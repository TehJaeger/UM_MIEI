...
-- * Altera os jogadores em funcao dos disparos que estao presentes

-- | procura se existe uma bala de canhao que conecte com algum tanque apos ser disparada
...
-- | pegando na lista inteira e apenas contando os canhoes e pelas funcoes anteriores calcula o dano causado nos jogadores
jogadorCanhao :: [Disparo] -> [Jogador] -> Mapa -> [Jogador]
jogadorCanhao [] _ a = []
jogadorCanhao _ [] a = []
jogadorCanhao (b:bs) pl a = danoTotal pl (movimentaCanhao b a bs pl)

-- | usa todas as funcoes anteriores e calcula a mudancas que os laser criam no jogador
jogadorLaser :: [Disparo] -> [Jogador] -> Mapa -> [Jogador]
jogadorLaser _ [] a = []
jogadorLaser (b:bs) pl a = danoTotal pl (movimentaLaser b a bs)

-- * Alterações do mapa pelo disparo que esta a ser avaliado 

-- | mapa laser pega nas funcoes feitas previamente e junta para calcular os efeitos da lista de laser na lista de disparos
mapaLaser :: [Disparo] -> Mapa -> Mapa
mapaLaser [] a =a
mapaLaser (b:bs) a = updateMapaTotal (movimentaLaser b a bs) a

-- | mapaCanhao pega nas funcoes feitas previamente e junta para calcular os efeitos da lista de canhao na lista de disparos
mapaCanhao :: [Disparo] -> Mapa -> [Jogador] -> Mapa
mapaCanhao _ a [] = a 
mapaCanhao [] a _ = a
mapaCanhao (b:bs) a pl = updateMapaTotal (movimentaCanhao b a bs pl) a

-- | Funções que recebem um mapa e disparos e atualizam o mapa dependendo do disparo que é dado
...
-- | usa a funcao anterior e com a lista total de disparos atualiza o mapa inteiro 
updateMapaTotal :: [Disparo] -> Mapa -> Mapa
updateMapaTotal [] a = a
updateMapaTotal d a = updateMapa (head d) a

-- * Disparos - funcoes que sao usadas para alteral os disparos 

-- | procura se ha uma colisao entre os disparos e um jogador 
...
-- | procura se ha uma colisao entre os disparos e todos os jogadores do jogo 
...
-- | procura se ha uma colisao entre todos os disparos e todos os jogadores do jogo 
danoTotal :: [Jogador] -> [Disparo] -> [Jogador]
danoTotal a [] = a
danoTotal a d = danoRonda a (head d)

-- | remove os disparos que são obsuletos e nessecitam de ser removidos
...
-- | procura se existe uma bala de canhao que conecte com algum disparo que nao seja ele mesmo apos ser disparada
...
-- | usa a funcao anterior e aplica a lista inteira  
...
-- | usa a funcao anterior e usa a lista inteira para a lista total 
finalCanhao :: [Disparo] -> [Disparo]
finalCanhao [] = []
finalCanhao (b:dps) = colisaoLista b dps 

-- | recebe dois disparos caso estes não sejam o mesmo verifica se um deles e destruido, caso contrario nada vai acontecer pois não havera uma colisapo
...
-- | pegando na funcao anterior corre-a por toda a lista 
finalLaser :: [Disparo] -> [Disparo]
finalLaser [] = []
finalLaser (b:dps) = colisaoLista b dps 

-- | recebe um mapa e um disparo para poder verifcar se ha uma parede indestrutivel e logo que pare a bala, caso contrario continua a ser aplicada ate isto acontecer
...
-- | recebe um mapa e um disparo para poder verifcar se ha qualquer tipo de parede que para a bala, caso contrario continua a ser aplicada ate isto acontecer
